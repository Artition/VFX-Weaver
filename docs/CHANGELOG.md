...
### Fixed
- **Block/item particles move smoothly and spin on a sane axis** — the model particles were submitted as geometry with their own pose interpolation, which read as jerky stepping and rotated around an odd pivot. They are now driven through client-side `BlockDisplay`/`ItemDisplay` entities, so vanilla interpolates position between ticks, `spin` is a yaw around the world Y axis with the model kept upright, and brightness/scale map onto the display's own brightness override and transformation. One display entity per live particle, removed on death/effect stop/world unload; the physics is unchanged.
