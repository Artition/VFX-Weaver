...
## Unreleased / Guide v46
### Fixed
- **Textured `surface_pattern` projections render on 1.21.11.** The texture resolver was guarded to `>=26.1` and left a neutral fallback on the `1.21.11` node; since the per-node depth work started registering the pass there, a texture never resolved, the `RESOLVED` flag bit was never set and the shader's fail-closed texture branch drew nothing. The resolver now has a real implementation on every node — 26.2/26.1.2 via the `sprite` AtlasManager, 1.21.11 via the remapped model AtlasManager and `TextureAtlas.getSprite` — with every source form (`block`/`item`/`atlas`/`standalone`) funnelling through one factory that sets the flag bit. See `docs/GUIDE.md` §2.1.
...
## Unreleased / Guide v45
### Changed
- **Scene depth now works on every supported line, not only 26.2.** `surface_pattern`, the depth/world field functions (`depth`, `depth_gradient`, `normal_facing`, `world_pos`) and the depth-needing mask families (a `world` leaf, an `aura` volume, a block leaf's `occlude`) were gated to 26.2 because the world reconstruction assumed 26.2's reversed depth. The per-node convention is proven from the real client jars — 26.2 is **reversed** (`glClipControl(GL_LOWER_LEFT, GL_ZERO_TO_ONE)` plus a near/far-swapped projection, near = 1, far = 0), 26.1.2 and 1.21.11 are **standard** (near = 0, far = 1) — and injected into the single shared shader source as the `VFX_DEPTH_REVERSED` define. The shader converts the raw sampled depth, picks the correct sky test and the correct block-occlusion comparison per node, so a depth-needing effect now renders on 26.1.2 and 1.21.11 as well as 26.2. Additive: 26.2 behaviour is unchanged, and a depth-needing effect still **fails closed** (zero coverage, no wrong sample) when no trustworthy depth is available. See `docs/GUIDE.md` §2.1 and §3.8.

## Unreleased / Guide v44
### Added
- **`surface.stitch` — opt-in planar (top-down) projection for `surface_pattern`.** A boolean in the structural `surface` block (default `false`): with `"stitch": true` every face samples the same top-down coordinate `p = world.xz`, so a wall pixel `(x, y, z)` shows exactly what the floor pixel at the wall base `(x, floor_y, z)` shows — the image's row at the wall line extruded vertically (a ring reaching a wall becomes two vertical stripes from its crossing points). Because `p` never depends on `world.y`, the seam is continuous for **any anchor height** (the anchor's Y no longer matters) and a vertical corner cannot tear the pattern; `min`/`max` become a height slab that bounds how far up the wall the image stretches. A ceiling is excluded (a top-down projection cannot light a down-facing surface). Deliberate trade-off: a wall shows a 1D slice (vertically constant colour columns), not a 2D unwrapped image. Additive and off by default — without it the hard floor/wall plane switch is byte-for-byte unchanged, so the built-in and every existing definition render exactly as before. See `docs/GUIDE.md` §2.1.
...
- **Per-instance particle state no longer leaks.** The vanilla-particle budgets, the aimed-particle lists and the `block_chain` rope simulations are keyed by instance id and were never removed when an effect stopped; they are now pruned against the live instance set every frame (the same prune the spark and block-particle engines already do).
- **A malformed field `texture` id can no longer disable every post effect in a layer.** A bad resource id threw from `Identifier.parse` into the layer-wide catch, skipping all post-processing that frame; it now fails closed for that one effect (neutral field value) with a one-time warning.
...
- **`surface_pattern` no longer slides when only the camera moves.** The anchor fell back to the camera when a definition authored no explicit `center`, so third-person (F5) or any camera-only motion shifted the projection along the ground. The anchor is now the effect instance's world position — a Java-API move, else its first declared `position` (`/vfx playat`), else the `pos_x/pos_y/pos_z` binds, else the local player for a player-anchored play; the camera is never consulted. An explicit `center` still wins, and effects played at a world point stay fixed there. See `docs/GUIDE.md` §2.1.
- **`pattern.texture` `aspect: preserve` now uses a standalone texture's own pixel aspect.** A standalone texture resolved with a hardcoded aspect of `1.0`, so `preserve` was a no-op for a non-square resource-pack PNG (it was stretched square); it now uses the texture's width/height, and a sheet cell's aspect is that times `rows / cols`, matching `preserve`'s documented meaning. Square textures are unchanged.
- **Textured `surface_pattern` figures are now crisp, not blurry.** The pattern texture is sampled with **NEAREST** filtering and no mipmaps (the same filtering vanilla uses for the block atlas), instead of bilinear, so a 16×16 or 64×64 pixel texture projected over `tile_scale` blocks reads as pixel art. One repeat spans `tile_scale` blocks (one texel = `tile_scale / pixels` blocks). Sprite-sheet cells are now mapped exactly and inset by half a texel, so a frame border no longer bleeds into the neighbouring cell (the cross-shaped artefact), `preserve` uses the **cell's** pixel aspect (a non-square `sheet` cell no longer stretches), and `frame` wraps into `0..cols*rows-1`. A sprite missing from a valid atlas is fail-visible again (the vanilla missing texture), matching the spec, rather than drawing nothing.
...
### Changed
- **Test/demo effects are iterated as a datapack, not baked into the mod.** The graph/field/mask/`surface_pattern`/spark demos plus the `particles`/`block_chain`/`ember`/`sparks` references can now be edited in the local test pack (`saves/<world>/datapacks/vfx_demos/`; a datapack definition overrides a same-id built-in) and picked up with `/reload` — no rebuild, restart or redeploy. The shipped built-ins are unchanged for now; deleting the demo copies from `src/main/resources/` is a separate follow-up. In that pack `vfxweaver:ember` is a smouldering ember (few, slow, long-lived, warm/dim, no bounce) while `vfxweaver:sparks` stays the fast bouncing spray.
