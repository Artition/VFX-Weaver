...
### Fixed
- **Physics `block_chain` ropes no longer stick in terrain** — a rope joint pushed inside a solid block is now pushed out along the smallest penetration axis and keeps its tangential velocity, so links that spawn inside blocks come free, links pushed into a wall slide along it, and a moving anchor drags the chain across the ground (instead of the joint being reverted to its previous position and freezing there). Multi-box collision shapes are approximated by their union box.
