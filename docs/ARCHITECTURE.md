...
- **`VFXServerEffects`** (server) — remembers every `VFXAPI.sendEffect` per player (`player → effectId → params/duration/easing/startMillis`). A player's effect age is **frozen at the instant they disconnect**, and on (re)join (`SYNC_DATA_PACK_CONTENTS`, after datapack sync) the still-active effects are re-sent with that frozen `elapsedTicks`, so a re-login resumes exactly where it left off (offline time never advances an effect). Finite effects that already ended are not resurrected; persistent (`-1`) and looping effects always come back, at the right phase. Bounded per player (`MAX_EFFECTS_PER_PLAYER`) and globally (`MAX_TRACKED_PLAYERS`, plus the `MAX_OFFLINE_MILLIS` offline expiry). Disabled while a Flashback replay is being played back (`Flashback.isInReplay()`) so effects already carried by the replay are not doubled.
...
| `MAX_TRACKED_PLAYERS` | 256 | `VFXServerEffects` — distinct players with remembered effects; the longest-offline is evicted |
| `MAX_OFFLINE_MILLIS` | 24 h | `VFXServerEffects` — a disconnected player's memory is pruned after this |
