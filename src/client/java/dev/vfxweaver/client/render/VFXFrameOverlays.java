	...
	public static void renderEffects(
		...
		for (VFXActiveEffect effect : effects) {
			try {
				...
				local.last().set(pose);
				// Model 0..1 space: the frame panel spans z = 0.96875..1.0 (15.5/16..16/16),
				// so the visible front face is at z ~ 0.97 - overlays draw just in front of it.
				if (tint) {
					submitNodeCollector.submitCustomGeometry(local, type, (captured, buffer) ->
						emitRect(buffer, captured, 0.0F, 0.0F, 1.0F, 1.0F, 0.98F, color, lightCoords));
				} else {
					float w = Math.min(Mth.clamp(effect.getParam("width", 0.05F), 0.01F, 0.2F) * 2.0F, 0.49F);
					submitNodeCollector.submitCustomGeometry(local, type, (captured, buffer) -> {
						emitRect(buffer, captured, 0.0F, 1.0F - w, 1.0F, 1.0F, 0.99F, color, lightCoords);  // top
						emitRect(buffer, captured, 0.0F, 0.0F, 1.0F, w, 0.99F, color, lightCoords);         // bottom
						emitRect(buffer, captured, 0.0F, w, w, 1.0F - w, 0.99F, color, lightCoords);       // left
						emitRect(buffer, captured, 1.0F - w, w, 1.0F, 1.0F - w, 0.99F, color, lightCoords); // right
					});
				}
